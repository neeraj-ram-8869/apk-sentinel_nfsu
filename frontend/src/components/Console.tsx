...
const NAV_IDS = NAV_ITEMS.map(i => i.id) as readonly NavId[];

// Sidebar views are panels onto the current scan, so they hang off whichever
// route is showing that scan rather than owning routes of their own.
function toLedgerItem(scan: StoredScan) {
  return {
    id: scan.id,
    file: { name: scan.fileName, size: scan.fileSize },
    result: scan.result,
    timestamp: scan.createdAt,
  };
}

function urlForView(view: NavId, scanId: string | null): string {
  // The dashboard is the scan ledger, not a panel onto one scan, so it always
  // lives at the root rather than under /report/<id>.
  if (view === "Dashboard") return "/";

  const base = scanId ? reportPath(scanId) : "/scan";
  if (view === "APK Scanner") return base;
...
export interface ConsoleProps {
  /** Which sidebar view this route opens on. */
  initialView?: NavId;
  /** Set by /report/[id] to restore a previously stored scan. */
  scanId?: string;
}

export default function Console({ initialView = "Dashboard", scanId }: ConsoleProps) {
  const router = useRouter();
  const pathname = usePathname();
  const searchParams = useSearchParams();

  // A view named in the query string wins, so any panel is linkable:
  // /report/<id>?view=Code%20Forensics
  const viewFromUrl = NAV_IDS.find(id => id === searchParams.get("view")) ?? initialView;
  const [activeNav, setActiveNav] = useState<NavId>(viewFromUrl);
  const [scanIdInUrl, setScanIdInUrl] = useState<string | null>(scanId ?? null);
  ...
  // Sidebar navigation now moves the URL too, so every panel is linkable and
  // the browser back button behaves.
  const navigate = useCallback((view: NavId) => {
    setActiveNav(view);
    router.push(urlForView(view, scanIdInUrl), { scroll: false });
  }, [router, scanIdInUrl]);

  const openScan = useCallback((id: string) => {
    router.push(reportPath(id), { scroll: false });
  }, [router]);

  // Restore the scan history on mount. Previously the ledger lived only in
  // React state, so a refresh emptied the dashboard.
  useEffect(() => {
    const stored = listScans();
    if (stored.length) {
      setLedger(stored.map(toLedgerItem));
    }
  }, []);

  // /report/[id] hydrates the console from stored state.
  useEffect(() => {
    if (!scanId) return;
    ...
    setCurrentFile({ name: stored.fileName, size: stored.fileSize });
    setAiNarrative(stored.narrative ?? "");
    setPhase("complete");
    setLogs([{ pct: 100, message: `Restored report for ${stored.fileName}`, level: "success" }]);
  }, [scanId]);
  ...
  const runAnalysis = useCallback(async (file: File, mockResult?: AnalyzedApkResult) => {
    ...
    try {
      ...
      const storedScan: StoredScan = {
        id: scanIdFor(result, file.name),
        createdAt: Date.now(),
        fileName: file.name,
        fileSize: file.size,
        packageName: result.manifest.packageName || "unknown",
        riskScore: generatedScore.score,
        riskTier: generatedScore.tier,
        result: { ...result, riskScore: generatedScore.score, riskTier: generatedScore.tier },
      };
      saveScan(storedScan);
      setScanIdInUrl(storedScan.id);
      // Give the finished report its own address, without adding a history
      // entry the back button would have to walk through.
      router.replace(reportPath(storedScan.id), { scroll: false });

      setLedger(prev => [
        toLedgerItem(storedScan),
        ...prev.filter((l: any) => l.id !== storedScan.id),
      ]);
      ...
      try {
        ...
        attachNarrative(storedScan.id, narrativeRes.text);
    ...
    <>
      ...
      <main className="flex-1 ml-0 md:ml-64 pt-16 md:pt-0 min-h-screen px-container-margin py-xl flex flex-col max-w-[1600px] mx-auto w-full">
        <div key={activeNav} className="animate-view flex-1 flex flex-col">
          ...
          {activeNav === "Dashboard" && (
            <DashboardView ledger={ledger} onNavigate={resetScan} onOpenScan={openScan} />
          )}
...
function ScannerView({
  ...
  scanIdInUrl, restoreFailed,
}: any) {
  const [linkCopied, setLinkCopied] = useState(false);

  const copyReportLink = useCallback(async () => {
    if (!scanIdInUrl || typeof window === "undefined") return;
    try {
      await navigator.clipboard.writeText(`${window.location.origin}${reportPath(scanIdInUrl)}`);
      setLinkCopied(true);
      window.setTimeout(() => setLinkCopied(false), 2000);
    } catch {
      // Clipboard is blocked in some embedded contexts; the URL bar still works.
      setLinkCopied(false);
    }
  }, [scanIdInUrl]);
  ...
  return (
    <div className="flex flex-col gap-5 w-full">
      {restoreFailed && (
        <div
          role="status"
          className="border border-outline-variant rounded-lg px-lg py-md bg-surface-container-lowest flex items-start gap-md"
        >
          <span className="material-symbols-outlined text-[20px] text-on-surface-variant">history</span>
          <div>
            <div className="font-label-mono text-on-surface" style={{ fontSize: "0.85rem", fontWeight: 600 }}>
              This report is not on this device.
            </div>
            <p className="text-on-surface-variant font-mono-base" style={{ fontSize: "0.72rem", margin: "4px 0 0" }}>
              Reports are stored on the device that ran the scan, so a link opened
              elsewhere cannot show it yet. Upload the APK below to analyse it here.
            </p>
          </div>
        </div>
      )}
              ...
              <div className="flex items-center gap-sm">
                {scanIdInUrl && (
                  <button
                    ...
                    title="Copy a link back to this report"
                    className="border border-outline-variant/60 text-primary font-label-mono px-md py-sm rounded hover:bg-primary/5 transition-colors flex items-center"
                    style={{ fontSize: "0.72rem" }}
                  >
                    <span className="material-symbols-outlined mr-xs text-[16px]">link</span>
